...
#endif
{
...
#if _NORMALMAP
	NormalsCombined = SampleNormals(uv * _NormalTiling, TIME, flowMap, _WaveSpeed * 0.1, slope);
	//return float4(NormalsCombined.xyz, 1);
	...
	*/
	...
	#if UNDERWATER_ENABLED
	//Hide caustics if viewing from underwater
	//causticsMask = lerp(causticsMask, 0, facing);
	#endif
	...
	#if UNDERWATER_ENABLED
	ApplyUnderwaterShading(color.rgb, alpha, wPos, _ShallowColor, _BaseColor, 1-facing);
	#endif
	...
	#if UNDERWATER_ENABLED
	ClipSurface(color.a, input.positionCS.z, wPos);
	#endif
